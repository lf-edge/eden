	...
	for _, tt := range networkInstanceTests {
		t.Run(tt.networkInstance.networkInstanceName, func(t *testing.T) {
			...
			t.Run("Process", func(t *testing.T) {
				err = ctx.InfoChecker(devUUID, map[string]string{"devId": devUUID.String(), "InfoContent.niinfo.networkID": tt.networkInstance.networkInstanceID}, einfo.HandleFirst, einfo.InfoAny, 300)
				if err != nil {
			...
			t.Run("Active", func(t *testing.T) {
				err = ctx.InfoChecker(devUUID, map[string]string{"devId": devUUID.String(), "InfoContent.niinfo.networkID": tt.networkInstance.networkInstanceID, "InfoContent.niinfo.activated": "true"}, einfo.HandleFirst, einfo.InfoAny, timeout)
				if err != nil {
