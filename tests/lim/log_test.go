...
func TestLogInGolang(t *testing.T) {
	...
	go func() {
		if err := eveNode.GetLogsFromAdam(
			func(logEntry *elog.FullLogEntry) bool {
				// Check if the log entry is a disconnect log
				if strings.Contains(logEntry.GetContent(), logToLookFor) {
					logInfof("Found the log containing '%s' string: %s", logToLookFor, logEntry.GetContent())
					foundTheLog <- struct{}{} // signal that the log was found
					return true               // return true to stop checking further logs
				}
	...
	logInfof("STEP 5: wait %v for the log to appear", logTimeout)
	timer := time.NewTimer(logTimeout)
	select {
	case <-foundTheLog:
		logInfof("The right log found before timeout")
	case <-timer.C:
		logFatalf("Timeout waiting for the log")
	}
}
