...
//createImageSFTP downloads image into EServer directory from http/https endpoint and calculates size and sha256 of image
func (exp *AppExpectation) createImageSFTP(id uuid.UUID, dsID string) *config.Image {
...
//checkImageSFTP checks if provided img match expectation
func (exp *AppExpectation) checkImageSFTP(img *config.Image, dsID string) bool {
...
//checkDataStoreSFTP checks if provided ds match expectation
func (exp *AppExpectation) checkDataStoreSFTP(ds *config.DatastoreConfig) bool {
...
//createDataStoreSFTP creates datastore, pointed onto EServer http endpoint
